...
import type { OpenItem } from '../srv/agents/integration-mocks.js';

test('mock extraction returns explicitly synthetic data with zero confidence', async () => {
  const payment = await extractPayment(Buffer.from('not a real PDF'));
...
test('mock payer resolution does not invent a customer match', async () => {
  const payment = await extractPayment(Buffer.alloc(0));
  const item: OpenItem = {
